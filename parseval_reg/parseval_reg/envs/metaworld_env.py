...
class MetaWorldSingleEnvSequence:
    def __init__(self,  change_freq=1e7, base_task_name=None, env_sequence="metaworld_sequence_set1", goal_hidden=True, normalize_obs="straight", normalize_avg_coef=0.0001,
        ...
        if base_task_name is not None:
            if goal_hidden:
                self.suite = metaworld.ML1(base_task_name)
            else:
                self.suite = metaworld.MT50()
            self.base_task_class = self.suite.train_classes[base_task_name]
    ...
    def make_task(self):
        ...
        if self.env_sequence is not None:
            ...
            # Pick a task name and upgrade it to v3 if needed
        ...
        # MetaWorld 3: must set the task explicitly
        if hasattr(self.suite, "train_tasks"):
            task = self.suite.train_tasks[0]
            temp_env.set_task(task)

        # MetaWorld 3: seed via reset(), not via __init__(seed=...)
        try:
            temp_env.reset(seed=self.current_seed)
        except TypeError:
            # For backward compatibility with old versions
            if hasattr(temp_env, "seed"):
                temp_env.seed(self.current_seed)
    ...
    def evaluate_agent(self, agent, num_eval_episodes=10):
        ...
        test_env = self.base_task_class()

        # Set the same task from the suite
        if hasattr(self.suite, "train_tasks"):
            test_env.set_task(self.suite.train_tasks[0])

        # Seed via reset (Gymnasium style)
        try:
            test_env.reset(seed=self.current_seed)
        except TypeError:
            if hasattr(test_env, "seed"):
                test_env.seed(self.current_seed)

        # Wrap for evaluation
        test_env = self._wrap_env(test_env, eval_mode=True)
